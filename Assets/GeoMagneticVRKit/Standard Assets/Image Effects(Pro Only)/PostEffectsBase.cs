...
public class PostEffectsBase : MonoBehaviour {
    ...
    public bool Dx11Support { get { return supportDX11; } }
    ...
    public Material CheckShaderAndCreateMaterial(Shader s, Material m2Create) {
        if (!s) {
            Debug.Log("Missing shader in " + gameObject.name);
            enabled = false;
            return null;
        }
        ...
        {
            NotSupported();
            Debug.Log("The shader " + s.ToString() + " on effect " + gameObject.name + " is not supported on this platform!");
            return null;
        }
        else
        {
    ...
    {
        Debug.LogWarning("The image effect " + gameObject.name + " has been disabled as it's not supported on the current platform.");
    }
